{
    Copyright (c) 2021 by Michael Van Canneyt (michael.vancanneyt@gmail.com)
...
Type
  ...
  Public
    Procedure Render(aContext : TMustacheContext; aOutput : TMustacheOutput; const aPrefix : String = ''; aLast : Boolean = False); overload; override;
  ...
  Public
    ...
    Procedure Parse(aParent : TMustacheElement); overload;
    Function Parse : TMustacheElement; overload;
  ...
  Private
    ...
    FCount : Integer;
    Function FindValue(Const aName : TMustacheString) : TJSONValue;
    function GetRootData: TJSONValue;
  Public
    Constructor Create(aJSON : TJSONValue; aCallback : TGetTextValueEvent); reintroduce; overload;
    Constructor Create(aJSON : String; aCallback : TGetTextValueEvent); reintroduce; overload;
    Constructor Create(aJSON : String); reintroduce; overload;
    ...
    Property RootData : TJSONValue read GetRootData;
  end;
  ...
  Public
    ...
    Procedure Render(aContext : TMustacheContext; aOutput : TMustacheOutput); overload; virtual;
    Function Render(aContext : TMustacheContext) : TMustacheString; overload;
    Function Render(const aJSON : TJSONValue) : TMustacheString; overload;
    Function Render(const aJSON : String) : TMustacheString; overload;
    Class function CreateMustache(aOwner: TComponent; aTemplate: TMustacheString): TMustache; virtual;
    Class Function Render(aTemplate : TMustacheString; const aJSON : String) : TMustacheString; overload;
  Published
...
function TMustache.Render(const aJSON: TJSONValue): TMustacheString;
...
function TMustache.Render(const aJSON: String): TMustacheString;

Var
  JSONData : TJSONValue;

begin
  JSONData:=TJSONObject.ParseJSONValue(aJSON,True,True);
  try
...
constructor TMustacheJSONContext.Create(aJSON: String);
begin
  Create(TJSONOBject.ParseJSONValue(aJSON),Nil);
end;

constructor TMustacheJSONContext.Create(aJSON: String; aCallback: TGetTextValueEvent);
begin
  Create(TJSONOBject.ParseJSONValue(aJSON),aCallBack);
end;
...
begin
  ...
  While (Result=Nil) and (aCount>=0) do
    ...
    With FStack[aCount] do
      if (Value is TJSONArray) and (Index>=0) and (Index<TJSONArray(Value).Count) then
        Result:=TJSONArray(Value).Items[Index].FindValue(N)
      else
        Result:=Value.FindValue(N);
    Dec(aCount);
    end;
end;

function TMustacheJSONContext.GetRootData: TJSONValue;
begin
  Result:=FStack[0].Value;
end;


constructor TMustacheJSONContext.Create(aJSON: TJSONValue;
...
begin
  With FStack[FCount-1] do
    ...
    if Value is TJSONArray then
      Result:=Index<TJSONArray(Value).Count
    else
      Result:=False;
    end;
end;
...
begin
  ...
  if S=Nil then
    Exit;
  if (S is TJSONArray) then
    begin
    if (TJSONArray(S).Count>0) then
      Result:=mstList
    end
  else if Not ((S is TJSONNull) or ((S is TJSONBool) and Not TJSONBool(S).AsBoolean)) then
     Result:=mstSingle;
...
begin
  ...
  if not Assigned(aJSON) then
    ...
    if (AJSON is TJSONNumber) and (Pos('.',TJSONNumber(aJSON).Value)>0) then
      Result:=FormatFloat('0.0###########',TJSONNumber(aJSON).AsDouble,FmtSettings)
    else
      Result:=aJSON.Value;
end;
...
procedure TMustacheSectionElement.Render(aContext: TMustacheContext; aOutput: TMustacheOutput; const aPrefix: String = ''; aLast : Boolean = False);
...
begin
  ...
  While (I<=Len) do
    begin
    While (I<=Len) and not CharInSet(aString[i],QuoteChars) do
      Inc(I);
    AddToResult;
    if I<=Len then
      Case aString[i] of
...
Var
  ...
  begin
    ...
    if isWhiteSpace then
      begin
      While (I<=Total) and CharInSet(aTemplate[I+clStop],[#13,#10]) do
        Inc(I);
      NewPos:=I;
